...
class Plantilla(QWidget):
    ...
    def __init__(self, base_actual, parent=None):
        ...
        self.ui.treeView_carga_maternidad.setModel(self.modelo_sesion)
        self.ui.treeView_carga_maternidad.setRootIsDecorated(False)
        self.ui.treeView_carga_maternidad.setAlternatingRowColors(True)  

        self.ui.treeView_carga_maternidad.setFocusPolicy(Qt.StrongFocus)
        ...
        (
            ...
            self.ui.treeView_carga_maternidad
        )
    ...
    def _init_autocompletado(self):
        self.autocomplete_campos = {
            self.ui.line_denominacion: "denominacion",
            #self.ui.line_fecha: "fecha",
            self.ui.line_documento: "documento",
        }
        ...
        for line in (
            self.ui.line_denominacion,
            #self.ui.line_expediente,
            self.ui.line_documento,
        ):
    ...
    def _cargar_fila_completa(self, id_registro):
        ...
        try:
            ...
            if "caratula" in campos_a_cargar:
                self.ui.line_caratula.setText(datos["caratula"] or "")
            ...
            if "observaciones" in campos_a_cargar:
                self.ui.line_observaciones.setText(datos["observaciones"] or "")            

            if "denominacion" in campos_a_cargar:
                self.ui.line_denominacion.setText(datos["denominacion"] or "")            

            if "documento" in campos_a_cargar:
                self.ui.line_documento.setText(datos["documento"] or "")                     

            if "fecha" in campos_a_cargar:
                self.ui.line_fecha.setText(datos["fecha"] or "")                                 

        finally:
            self._actualizando = False
            self.lista_autocomplete.hide()

        self.ui.line_fecha.setFocus()
    ...
    def _configurar_navegacion_enter(self):
        campos = [
            self.ui.line_caja,
            self.ui.line_estado,
            self.ui.line_caratula,
            self.ui.line_expediente,
            self.ui.line_ingreso,
            self.ui.line_egreso,
            self.ui.line_observaciones,
            self.ui.line_denominacion,           
            self.ui.line_documento,
            self.ui.line_fecha,           
        ]
    ...
    def _validar_campos_obligatorios(self):
        ...
        campos = [
            ...
            ("Carátula", self.ui.line_caratula),
            ("Expediente", self.ui.line_expediente),
            ("Ingreso", self.ui.line_ingreso),
            ("Egreso", self.ui.line_egreso),
            ("Observaciones", self.ui.line_observaciones),
            ("Denominación", self.ui.line_denominacion),           
            ("Documento", self.ui.line_documento),
            ("Fecha", self.ui.line_fecha),            
        ]
    ...
    def guardar_registro(self):
        ...
        try:
            datos = [
                self.ui.line_caja.text(),
                self.ui.line_estado.text(),
                self.ui.line_caratula.text(),
                self.ui.line_expediente.text(),
                self.ui.line_ingreso.text(),
                self.ui.line_egreso.text(),
                self.ui.line_observaciones.text(),                
                self.ui.line_denominacion.text(),                
                self.ui.line_documento.text(),
                self.ui.line_fecha.text(),               
            ]
    ...
    def limpiar_campos(self):
        self.ui.line_denominacion.clear()
        #self.ui.line_expediente.clear()
        #self.ui.line_estado.clear()
        #self.ui.line_caratula.clear()
        #self.ui.line_ingreso.clear()
        #self.ui.line_egreso.clear()
        self.ui.line_documento.clear()
        self.ui.line_fecha.clear()        
        #self.ui.line_observaciones.clear()        
        #self.ui.line_caja.clear()

        self.ui.line_denominacion.setFocus()

    def eliminar_registro(self):
        index = self.ui.treeView_carga_maternidad.currentIndex()
